...
def getAllComponents(jsondata: dict):
    ...
    while queue:
        ...
        if 'node' in currentNode:
            if type(currentNode['node']).__name__ == 'dict' or type(currentNode['node']).__name__ == 'OrderedDict':
...
def getOutput(question: str):
    openai.api_key = "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"
    start_sequence = "\nA:"
    restart_sequence = "\n\nQ: "
    question = question + "Please give me an example that I can fill it in the text field. Your response should be put your response into the []"
...
def show_hint(res: list, hint_text: str):
    ...
    # cmd = cmd.replace('{x1}', str(x1)).replace('{y1}', str(y1)).replace('{x2}', str(x2)).replace('{y2}', str(y2)).replace('{text}', hint_text)
    # print(cmd)
    # os.system(cmd)
    os.system(tap_cmd)
    time.sleep(1)
